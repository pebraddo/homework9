...
You should add (using the first recipe from homework 8 with no quadratics nor interactions):

• a (tuned) LASSO model: where LASSO is a similar model to the MLR model but involves a tuning parameter.

(note: I got all the same rmse values for every tuning value)
...
Let's pull out the 'best' model using select_best() and finalize_workflow().
...
LASSO_model <- LASSO_wkf |>
  fit(data = new_bike_data)
tidy(LASSO_model)
```
...
tree_wkf <- workflow() |>
  ...
  add_model(tree_mod)
temp_tree <- tree_wkf |> 
  tune_grid(resamples = seoul_10_fold) #uses defaults from dials
temp_tree |> 
  collect_metrics()
...
• Extract the final model fits for each type and report a summary of each model
...
∗ For the random forest model, this is a bit complicated. Check this out and see if you can getit to work.
